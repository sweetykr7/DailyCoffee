...
export default function AdminProductEditPage() {
    ...
    const fetchData = async () => {
      try {
        const [productRes, catRes] = await Promise.all([
          api.get<Product>(`/admin/products`, { page: '1', limit: '1' }),
          api.get<Category[]>('/admin/categories'),
        ]);

        if (catRes.success && catRes.data) setCategories(catRes.data);

        // Fetch single product by getting all and filtering, or use direct endpoint
        // Actually let's fetch the product directly from the public endpoint
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await api.put<Product>(`/admin/products/${id}`, body);
      if (res.success) {
        router.push('/admin/products');
      } else {
        setError(res.error || '상품 수정에 실패했습니다.');
      }
    } catch {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="rounded-xl bg-white p-6 shadow-sm">
        <h2 className="mb-6 text-lg font-semibold text-coffee">상품 수정</h2>
